...
### 选择运算（Selection）
选择 $$ \sigma_{\text{条件}}(R) $$ 从关系 R 中选择满足条件的元组。
...
### 投影运算（Projection）
投影 $$ \pi_{\text{属性列表}}(R) $$ 从关系 R 中选择指定属性，去除重复。
- 形式化：$$ \pi_{A_1, A_2, \dots, A_k}(R) = \{ t[A_1, A_2, \dots, A_k] \mid t \in R \} $$。
...
- **θ-连接**：$$ R \bowtie_\theta S = \{ t[r] \cup t[s] \mid t[r] \in R, t[s] \in S, \theta(t[r], t[s]) \} $$，θ 是比较条件。
...
### 换名操作（Rename）
换名 $$ \rho_{\text{NewName}}(R) $$ 或 $$ \rho_{A \to B}(R) $$ 重命名关系或属性。
