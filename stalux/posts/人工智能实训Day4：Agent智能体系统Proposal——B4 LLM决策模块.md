...
本项目面向 **本地私有化环境下的个人智能文档处理与数据分析** 场景，目标用户为需要在本地处理文件且关注数据隐私的研究人员、数据分析师、开发者等，尝试解决**在无外部API依赖下，本地部署的4B参数小模型难以稳定执行自然语言驱动的工具调用决策**的问题。整体系统包括B1 Agent Runtime（运行时管理）、B2 Skill（技能函数层）、B3 Tool Layer（工具调用层）、B4 LLM决策模块（本模块）、B5 Memory（记忆系统）等模块，最终希望实现**用户在完全离线的本地环境中，通过自然语言指令自动调用文件读取、数据分析、信息检索等本地工具，完成从任务理解到结果交付的端到端智能处理**。
