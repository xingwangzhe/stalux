...
function isDuplicateAbbrlink(
  ...
  return Object.entries(timestamps).some(
    ([path, data]: [string, FileTimestamp]) => path !== currentFilePath && data.abbrlink === abbrlink
  );
}
...
export function remarkModifiedAbbrlink() {
  return function (tree: Root, file: VFile): void {
    ...
    if (!filepath) {
      console.warn('[remark-abbrlink] 无法获取文件路径');
      return;
    }
    ...
    if (!file.data.astro?.frontmatter) {
      console.warn('[remark-abbrlink] 文件没有 frontmatter');
      return;
    }
    
    // 检查时间戳数据中是否已有 abbrlink
    if (timestamp?.abbrlink) {
      // 如果已有 abbrlink，直接使用
    ...
    // 使用统一的 abbrlink 生成函数
    const abbrlink = generateUniqueAbbrlink(filepath, createdTime, timestamps);
